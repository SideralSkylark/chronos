...
public class TimetableConstraintProvider implements ConstraintProvider {
  ...
  // HC9: Optional subjects must be taght at the same time on the same day only
  // on diferent rooms
  private Constraint optionalGroupSameTimeslot(ConstraintFactory factory) {
    return factory
        .forEachUniquePair(
            LessonAssignment.class,
  ...
  private boolean areOptionalPair(LessonAssignment l1, LessonAssignment l2) {
    return l1.getOptionalGroupId() != null
        && l1.getOptionalGroupId().equals(l2.getOptionalGroupId());
  }
}
