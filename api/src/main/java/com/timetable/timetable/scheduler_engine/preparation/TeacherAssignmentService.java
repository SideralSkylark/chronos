...
public class TeacherAssignmentService {
  ...
  /**
   * Assigns the least-loaded eligible teacher to a subject. Falls back to a phantom teacher if all
   * eligible teachers would exceed their contract limits.
   */
  public TeacherAssignmentResult assignTeacher(
      Subject subject, List<CohortSubject> existingInSemester, PhantomTeacherPolicy policy) {
  ...
  /**
   * Finds or creates a phantom teacher with enough capacity for {@code hoursNeeded}. Phantom slots
   * are indexed (PHANTOM_{subjectId}, PHANTOM_{subjectId}_2, ...).
   */
  private TeacherAssignmentResult assignPhantom(
      Subject subject, int hoursNeeded, List<CohortSubject> existingInSemester) {
    ...
    for (int slot = 1; slot <= MAX_PHANTOM_SLOTS; slot++) {
      ...
      if (currentLoad + hoursNeeded <= limit) {
        log.warn(
            "Using phantom teacher {} for subject {} ({}h + {}h)",
            username,
            subject.getName(),
            currentLoad,
            hoursNeeded);
        return new TeacherAssignmentResult(
            candidate, true, "Phantom teacher used for: " + subject.getName());
      }
    }
  ...
  private ApplicationUser findOrCreatePhantom(String username) {
    return userService.findByUsername(username).orElseGet(() -> createPhantom(username));
  }
  ...
  private Map<Long, Integer> computeWorkload(
      Set<ApplicationUser> eligible, List<CohortSubject> existingInSemester) {

    Set<Long> eligibleIds =
        eligible.stream().map(ApplicationUser::getId).collect(Collectors.toSet());
  ...
  private boolean fitsWithinLimit(
      ApplicationUser teacher, Map<Long, Integer> workload, int hoursNeeded) {
