...
/**
 * Persists a solved TimetableSolution into Timetable + ScheduledClass. Delegates entity resolution
 * to the existing PersistenceMapper.
 */
@Service
...
public class TimetablePersistenceService {
  ...
  public Timetable saveSolution(TimetableSolution solution) {
    ...
    log.info(
        "Persisting timetable {}.{} — {} lessons ({} unassigned)",
        year,
        semester,
    ...
    try {
      ...
      notificationService.notify(
          currentUserId, "Horário " + year + "·" + semester + "º semestre gerado com sucesso.");
    ...
    log.info(
        "Saved timetable id={} for {}.{} with {} scheduled classes",
        saved.getId(),
        year,
        semester,
        classes.size());
