 ...
 *
 * <ul>
 *   <li>Retrieving the logged-in user's profile
 *   <li>Updating user details
 *   <li>Deleting user account
 * </ul>
 *
 * <p>All endpoints require a valid authenticated session. Role-based access control is enforced
 * where applicable.
 *
...
public class UserController {
   ...
   *
   * <p>Retrieves user details based on the authentication context. Converts the internal {@link
   * ApplicationUser} to {@link UserResponseDTO}.
   *
  ...
  public ResponseEntity<ApiResponse<UserResponse>> getUserDetails() {
    ...
    return ResponseFactory.ok(
        userService.getAuthenticatedUserProfile(), "User profile retrieved successfully");
  }

  /**
   * Update the authenticated user's profile information.
   *
   * <p>Updates basic fields like username, email, and enabled status. Note that only authenticated
  ...
  public ResponseEntity<ApiResponse<UserResponse>> updateUserDetails(
    ...
    return ResponseFactory.ok(
        userService.updateAuthenticatedUserProfile(payload), "User profile updated successfully");
  }

  /**
   * Permanently delete the currently authenticated user's account.
   *
   * <p>This operation is irreversible and requires valid authentication.
   *
