...
public class CourseService {
  ...
  public CourseResponse createCourse(CreateCourseRequest createRequest) {
    ...
    Map<Integer, Integer> cohorts =
        createRequest.expectedCohortsPerYear() != null
  ...
  public Page<CourseListResponse> findAllWithSubjectCount(Pageable pageable) {
    ...
    Map<Long, Long> subjectCounts =
        subjectRepository.countByCourseIds(ids).stream()
            .collect(Collectors.toMap(row -> (Long) row[0], row -> (Long) row[1]));

    Map<Long, Course> courseById =
        courses.stream().collect(Collectors.toMap(Course::getId, Function.identity()));

    List<CourseListResponse> responses =
        ids.stream()
            .map(
                id ->
                    CourseListResponse.from(courseById.get(id), subjectCounts.getOrDefault(id, 0L)))
            .toList();
  ...
  public Page<CoordinatorOption> getAvailableCoordinators(Pageable pageable) {
    return userRepository
        .findAllByRole(UserRole.COORDINATOR, pageable)
        .map(CoordinatorOption::from);
  }
  ...
  public CourseResponse updateCourse(Long id, UpdateCourseRequest updateRequest) {
    ...
    if (!course.getName().equals(updateRequest.name())
        && courseRepository.existsByName(updateRequest.name())) {
  ...
  public void deleteCourse(Long id) {
    ...
    if (!courseRepository.existsById(id)) {
      throw new IllegalArgumentException("No course with id: %d".formatted(id));
    }
  ...
  private void validateCohortConfig(int years, Map<Integer, Integer> cohorts) {
    cohorts.forEach(
        (year, value) -> {
          ...
          if (value <= 0) {
            throw new IllegalArgumentException("Cohort number must be positive");
          }
        });
  }
}
