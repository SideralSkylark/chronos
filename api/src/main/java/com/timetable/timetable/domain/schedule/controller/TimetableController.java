...
public class TimetableController {
  ...
  public ResponseEntity<ApiResponse<TimetableResponse>> create(
    ...
    return ResponseFactory.ok(
        timetableService.createTimetable(request), "Timetable created successfully.");
  }

  @GetMapping
  public ResponseEntity<ApiResponse<PagedModel<TimetableResponse>>> getAll(Pageable pageable) {
    return ResponseFactory.ok(
        new PagedModel<>(timetableService.getAll(pageable)), "Timetables fetched successfully.");
  }

  @GetMapping("/{id}")
  public ResponseEntity<ApiResponse<TimetableResponse>> getById(@PathVariable Long id) {
    return ResponseFactory.ok(timetableService.getById(id), "Timetable fetched successfully.");
  }
  ...
  public ResponseEntity<ApiResponse<List<CandidateTeacherResponse>>> getReplacementCandidates(
    ...
    return ResponseFactory.ok(
        timetableService.getReplacementCandidates(lessonId), "Teachers fetched successfully.");
  }

  @GetMapping("/periods")
  public ResponseEntity<ApiResponse<List<AcademicPeriodDto>>> getPeriods() {
    return ResponseFactory.ok(
        timetableService.getAvaliablePeriods(), "Periods fetched successfully.");
  }

  @PutMapping("/{id}")
  public ResponseEntity<ApiResponse<TimetableResponse>> update(
      @PathVariable Long id, @Valid @RequestBody UpdateTimetableRequest request) {
    return ResponseFactory.ok(
        timetableService.updateTimetable(id, request), "Timetable updated successfully.");
  }
  ...
  public ResponseEntity<ApiResponse<TimetableResponse>> submit(@PathVariable Long id) {
    return ResponseFactory.ok(
        timetableService.submitForApproval(id), "Timetable submitted for approval.");
  }

  @PostMapping("/{id}/approve")
  @Transactional
  public ResponseEntity<ApiResponse<TimetableResponse>> approve(@PathVariable Long id) {
    return ResponseFactory.ok(timetableService.approve(id), "Timetable approved.");
  }
  ...
  public ResponseEntity<ApiResponse<TimetableResponse>> reject(@PathVariable Long id) {
    return ResponseFactory.ok(
        timetableService.reject(id), "Timetable rejected and reverted to draft.");
  }

  @PostMapping("/{id}/publish")
  @Transactional
  public ResponseEntity<ApiResponse<TimetableResponse>> publish(@PathVariable Long id) {
    return ResponseFactory.ok(timetableService.publishTimetable(id), "Timetable published.");
  }

  @PreAuthorize("hasAnyRole('ADMIN', 'DIRECTOR', 'ASISTENT', 'COORDINATOR')")
  @PatchMapping("/lessons/{lessonId}/reassign-teacher")
  public ResponseEntity<ApiResponse<TimetableResponse>> reasignTeacher(
      @PathVariable Long lessonId, @RequestBody ReasignTeacherRequest request) {
    return ResponseFactory.ok(
        timetableService.reasignTeacher(lessonId, request), "Teacher reasigned successfully");
  }
