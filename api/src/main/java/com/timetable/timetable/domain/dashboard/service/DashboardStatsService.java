...
public class DashboardStatsService {
  ...
  public DashboardStatsDTO computeStats(Integer year, Integer semester) {
    ...
    if (year != null && semester != null) {
      ...
      Timetable latest =
          timetableRepository.findFirstByOrderByAcademicYearDescSemesterDesc().orElse(null);
      if (latest != null) {
    ...
    List<Cohort> cohorts =
        cohortRepository.findByAcademicYearAndSemester(targetYear, targetSemester);
    ...
    double averageCohortSize =
        totalCohortCount > 0 ? (double) totalCohortDemand / totalCohortCount : 0.0;

    Map<Integer, Long> cohortsByYearCount =
        cohorts.stream().collect(Collectors.groupingBy(Cohort::getYear, Collectors.counting()));
    Map<Integer, Long> studentsByYear =
        cohorts.stream()
            .collect(
                Collectors.groupingBy(
                    Cohort::getYear, Collectors.summingLong(Cohort::getStudentCount)));

    List<YearBreakdownDTO> cohortsByYear =
        cohortsByYearCount.keySet().stream()
    ...
    List<CohortSubject> cohortSubjects =
        cohortSubjectRepository.findByAcademicYearAndSemester(targetYear, targetSemester);
    Map<ApplicationUser, Long> hoursPerTeacher =
    ...
    Map<ApplicationUser, Integer> sessionsPerTeacher =
        cohortSubjects.stream()
            .collect(
                Collectors.groupingBy(
                    CohortSubject::getAssignedTeacher,
    ...
    double avgHoursPerTeacher =
        totalTeachers > 0 ? (double) totalAssignedHours / totalTeachers : 0.0;
    ...
    // Top Courses
    List<CourseRankDTO> topCoursesByCohorts =
        cohorts.stream()
            ...
            .map(
                entry ->
                    new CourseRankDTO(
                        entry.getKey().getId(), entry.getKey().getName(), entry.getValue().size()))
            .sorted(Comparator.comparing(CourseRankDTO::getCohortCount).reversed())
            .limit(5)
            .toList();

    // Teacher workload rankings
    List<TeacherWorkloadDTO> allTeacherWorkloads =
        teachers.stream()
            .map(
                t -> {
                  long hours = hoursPerTeacher.getOrDefault(t, 0L);
                  ...
                  return new TeacherWorkloadDTO(
                      t.getId(), t.getUsername(), hours, max, overloaded, sessions, estimatedHours);
                })
            .toList();

    double totalEstimatedHours =
        allTeacherWorkloads.stream()
    ...
    List<TeacherWorkloadDTO> mostLoadedTeachers =
        allTeacherWorkloads.stream()
            .sorted(Comparator.comparing(TeacherWorkloadDTO::getTotalHours).reversed())
            .limit(5)
            .toList();

    List<TeacherWorkloadDTO> leastLoadedTeachers =
        allTeacherWorkloads.stream()
    ...
    List<OversizedCohortDTO> oversizedCohorts =
        cohorts.stream()
            ...
            .map(
                c -> {
                  int compatible =
                      (int)
                          rooms.stream()
                              .filter(r -> r.getCapacity() >= c.getStudentCount())
                              .count();
                  return new OversizedCohortDTO(
                      c.getId(),
                      c.getDisplayName(),
                      c.getStudentCount(),
                      c.getStudentCount(),
    ...
    List<FragmentationRiskCohortDTO> fragmentationRisk =
        cohorts.stream()
            .filter(
                c -> {
                ...
                c -> {
                  ...
                  int maxCompCap =
                      rooms.stream()
                          ...
                          .orElse(0);
                  double utilization = (double) headcount / maxCompCap * 100;
                  return new FragmentationRiskCohortDTO(
                      c.getId(), c.getDisplayName(), headcount, maxCompCap, utilization);
                })
            .toList();
    ...
    long mediumRooms =
        rooms.stream().filter(r -> r.getCapacity() >= 30 && r.getCapacity() <= 54).count();
    ...
    long mediumCohorts =
        cohorts.stream()
            .filter(c -> c.getStudentCount() >= 30 && c.getStudentCount() <= 54)
            .count();
    ...
    if (totalRoomsCount > 0 && totalCohortsCount > 0) {
      roomTierDistribution.add(
          new RoomTierDTO(
              "Pequena (≤29)",
              smallRooms,
              (smallRooms / totalRoomsCount) * 100,
      ...
      roomTierDistribution.add(
          new RoomTierDTO(
              "Média (30–54)",
              mediumRooms,
              (mediumRooms / totalRoomsCount) * 100,
              (mediumCohorts / totalCohortsCount) * 100,
              (mediumRooms < mediumCohorts) ? "RED" : "GREEN"));
      roomTierDistribution.add(
          new RoomTierDTO(
              "Grande (≥55)",
              largeRooms,
              (largeRooms / totalRoomsCount) * 100,
    ...
    if (totalRoomsCount > 0 && totalCohortsCount > 0) {
      distributionMismatches.add(
          new DistributionMismatchDTO(
              "Pequena dimensão",
              (smallCohorts / totalCohortsCount) * 100,
              (smallRooms / totalRoomsCount) * 100));
      distributionMismatches.add(
    ...
    dto.setRoomScarcityNote(
        oversizedCohorts.isEmpty()
            ? "Não foram detectadas turmas sem sala compatível."
            : "Existem turmas que excedem a capacidade de todas as salas.");
    dto.setDistributionMismatches(distributionMismatches);
    dto.setDistributionMismatchNote(
        mediumRooms < mediumCohorts || largeRooms < largeCohorts
