 ...
 *
 * <ul>
 *   <li>Creating new refresh tokens during login or token rotation
 *   <li>Validating token existence, expiration, and revocation state
 *   <li>Rotating tokens securely
 *   <li>Revoking or deleting tokens
 *   <li>Querying token-related data (by token, ID, or user)
 * </ul>
 *
 ...
 *   <li>User agent (browser/device info)
 *   <li>Expiration date
 *   <li>Revocation status
 * </ul>
 *
 * <p>Default expiration is configurable via the property {@code auth.refresh-token-validity-days}
 * (default = 7).
 *
...
public class RefreshTokenService {
   ...
   *
   * <p>The token includes client IP and user agent, and is set to expire based on the configured
   * validity period.
   *
  ...
  public boolean isTokenValid(String token) {
    return refreshTokenRepository.findByToken(token).map(this::isTokenUsable).orElse(false);
  }
   ...
   *
   * <p>This is typically done after successful access token renewal.
   *
   ...
   *
   * <p>Primarily used for testing purposes to simulate token expiration.
   *
